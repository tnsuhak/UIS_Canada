export default async (request, context) => {
  ...
  html = html.replace(/<section class="sec sec--tint" id="outcomes">[\s\S]*?<\/section>/, (section) => {
    section = section.replace(/<a href="\/outcomes\/2026-university-offers-scholarships\.html"[^>]*>2026 대입·장학금 결과 자세히 보기 →<\/a>/g, "");
    section = section.replace(/<p class="reveal uis-section-footer-actions uis-outcomes-actions"[\s\S]*?<\/p>/g, "");
  ...
  html = html.replace(/<section class="sec"><div class="wrap"><div class="sec__hd reveal"><span class="tag">생활과 시설<\/span>[\s\S]*?<\/section>/, (section) => {
    ...
    section = section.replace(/<p class="reveal uis-section-footer-actions uis-accommodation-actions"[\s\S]*?<\/p>/g, "");
    section = section.replace(/<p class="reveal uis-section-footer-actions uis-clubs-actions"[\s\S]*?<\/p>/g, "");
